...
;-----FAT 16 HEADER----------------------------------------------------------------
    ...
    OEMIdentifier:     db    'M:OS    ' ;0x003 size of 8 Bytes (double byte)
    BytesPerSector:    dw    0x200 ;512 bytes (fixed by disk)
    SectorsPerCluster: db    0x80 ; 128 Sectors per Cluster
    ReservedSectors:   dw    200 ; 200 Reserverd Sectors (Kernel etc.) wont be stored in an file
    FATCopies:         db    0x02 ; two Copies (og and backup)
    RootDirEntries:    dw    0x40 ; 64 RootDirEntries
    NumSectors:        dw    0x00 ; Total logical sectors
    MediaType:         db    0xF8 ; Fixed disk (i.e., typically a partition on a hard disk)
    SectorsPerFat:     dw    0x100 ; 256 Logical sectors per File Allocation Table for FAT16
    SectorsPerTrack:   dw    0x20 ; 32 Number of sectors per track. 
    NumberOfHeads:     dw    0x40 ; 64 Number of heads or sides on the storage media.
    HiddenSectors:     dd    0x00 ; Number of hidden Sectors
    SectorsHuge:       dd    0x773594 ; 7812500 Bytes
    ; Extended BPB (DOS 4.0)
    DriveNumber:       db    0x80
    WinNTBit:          db    0x00
    Signature:         db    0x29
    VolumeID:          dd    0xD105
    VolumeIDString:    db    "M:OS Loader" ;exact 11 bytes
